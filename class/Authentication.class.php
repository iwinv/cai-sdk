...
class Authentication
{
	...
	/**
	 * @var string 인코딩된 파일 요청주소
	 */
	public static $encodefilesUrl ;
	/**
	 * @var string 인코딩 rule 요청주소
	 */
	public static $ruleUrl ;
	/**
	 * @var string 폴더키
	 */
	public $folderKey ;
	...
	function __construct ( $setting )
	{
		...
		self::$downloadUrl = self::$apiDomain . 'image' . '/' ;
		self::$ruleUrl = self::$apiDomain . 'rules' . '/' ;
		self::$encodefilesUrl = self::$apiDomain . 'encodefiles' . '/' ;
	...
	public function encodingFilesSelect ( $token = '' , $filesKey )
	{
		...
		$headers[] = 'Authorization:' . $_token ;
		$re = self::curl ( self::$encodefilesUrl . $filesKey , $headers , 'GET' ) ;
	...
	* @return
	*/
	public function ruleSelect ( $token = '' )
	{
		$_token = $token ? $token : ( $this -> token ? $this -> token : NULL ) ;
		if ( ! $_token )
			return 'No token' ;
		...
		$headers[] = 'Authorization:' . $_token ;
		$re = self::curl ( self::$ruleUrl , $headers , 'GET' ) ;
		return $this -> returnMsg ( $re , __FUNCTION__ ) ;
	 }
}
