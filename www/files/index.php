				...
				<tbody>
					<?php if ( $filesList ) : ?>
						...
						<tr>
							...
							<td><?=$v -> tag?></td>
							<td><?=$v -> date_insert?></td>
							<td><a href='encoding/index.php?key=<?=$k?>'>인코딩 이미지</a></td>
						</tr>
						<?php endforeach ; ?>
					<?php else : ?>
					<tr>
...
$ ( document ).ready ( function () {
	...
	*/
	$( "button[name=btnUpload]" ) . click ( function () {
		...
		form.delete('files[]');
		Array.from(fileInput).forEach(function(v,k){
			var fileSend = new File ( [v] , btoa(encodeURIComponent(v.name)) , { type : v.type } );
